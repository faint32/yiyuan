...
import com.yiyuan.player.utils.ConfigUtils;
import com.yiyuan.player.utils.NetworkUtils;
import com.yiyuan.player.utils.StorageUtils;
...
public class DownloadManager extends Thread {
    ...
    public DownloadManager(Context context, String savePath) {
    ...
    public void addTask(String url, String savePath) {
        ...
        try {
            addTask(newDownloadTask(url, savePath));
    ...
    public void checkUncompleteTasks() {
        ...
        if (urlList.size() >= 0) {
            for (int i = 0; i < urlList.size(); i++) {
                addTask(urlList.get(i), savePath);
            }
        }
    }
    ...
    public synchronized void pauseTask(DownloadTask task) {
        ...
        if (task != null) {
            ...
            try {
                ...
                task = newDownloadTask(url, savePath);
                mPausingTasks.add(task);
            } catch (MalformedURLException e) {
                e.printStackTrace();
            }
     ...
     */
    private DownloadTask newDownloadTask(String url, String savePath) throws MalformedURLException {
        ...
        return new DownloadTask(mContext, url, savePath, taskListener);
    }
