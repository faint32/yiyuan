...
import com.yiyuan.player.vlc.interfaces.IPlayerControl;
import com.yiyuan.player.vlc.interfaces.OnPlayerControlListener;

public class VideoPlayerActivity extends YiYuanPlayerActivity implements IVideoPlayer ,IPlayerControl{
     ...
     * 隐藏控制面板
     */
    private static final int HIDE_CONTROL_PANEL= 7;
    
    /**
     * 播放时改变进度条
     */
    private static final int CHANGE_SEEKBAR_PROGRESS = 8;
    
    /**
     * 改变播放时间
     */
    private static final int CHANGE_PLAYER_TIME = 9;
...
//    private static final int OVERLAY_INFINITE = 3600000;
    ...
    public void onCreate(Bundle savedInstanceState) {
         ...
         * 当前视频播放的进度
         */
        mTime = (TextView) findViewById(R.id.player_time_current);
        
        /**
         * 视频的总长度
         */
        mLength = (TextView) findViewById(R.id.player_time_length);

        mInfo = (TextView) findViewById(R.id.player_overlay_info);
        /**是否播放，暂停按钮*/
        mPlayPause = (ImageButton) findViewById(R.id.bottom_btn_playpause);
        mPlayPause.setOnClickListener(mPlayPauseListener);
        
        mPlayerTopBar = findViewById(R.id.player_top_bar);
        
        mPlayerBottomBar = findViewById(R.id.player_bottom_bar);
        ...
        if(VLCUtil.isGingerbreadOrLater() && chroma.equals("YV12")) {
    ...
    private final OnClickListener mPlayPauseListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if(mLibVLC.isPlaying()) {
				pause();
				mPlayPause.setImageResource(R.drawable.ic_play_media_style);
			}else {
				play();
				mPlayPause.setImageResource(R.drawable.ic_stop_media_style);
			}
		}
	};
    ...
    protected void onStart() {
        ...
        showOverlay();
        mSwitchingView = false;
        System.out.println("onstart videoplayer");
        progressHandler.sendEmptyMessage(CHANGE_PLAYER_TIME);
        progressHandler.sendEmptyMessage(CHANGE_SEEKBAR_PROGRESS);
    }
...
//                editor.putLong(PreferencesActivity.VIDEO_RESUME_TIME, time);
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            else if (action.equalsIgnoreCase(YiYuanPlayerApplication.SLEEP_INTENT)) {
                finish();
            }
        }
    };
    ...
    private void hideControlPanel() {
    	if(mPlayerTopBar.getVisibility() == View.VISIBLE) {
    ...
    private void showControlPanel() {
    	if(mPlayerTopBar.getVisibility() == View.GONE) {
    		mPlayerTopBar.setVisibility(View.VISIBLE);
    		sendHideControlPanelMessage();
    	} else{
    		mPlayerTopBar.setVisibility(View.GONE);
    	}
    	if(mPlayerBottomBar.getVisibility() == View.GONE) {
    ...
    private final Handler progressHandler = new VideoPlayerChangeProgressHandler(this);
    /**
     * 播放进度改变的handler
     * @author lianjie
     *
     */
    private class VideoPlayerChangeProgressHandler extends WeakHandler<VideoPlayerActivity> {

		public VideoPlayerChangeProgressHandler(VideoPlayerActivity owner) {
			super(owner);
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case CHANGE_SEEKBAR_PROGRESS:
				
				break;

			case CHANGE_PLAYER_TIME:
				mSeekbar.setProgress((int)mLibVLC.getTime());
				progressHandler.sendEmptyMessageDelayed(CHANGE_PLAYER_TIME, 100);
				break;
			}
		}
    ...
    private class VideoPlayerHandler extends WeakHandler<VideoPlayerActivity> {
...
//                    }
    ...
    private void doSeekTouch(float coef, float gesturesize, boolean seek) {
        ...
        showInfo(String.format("%s%s (%s)", jump >= 0 ? "+" : "",VLCUtil.millisToString(jump), VLCUtil.millisToString(time + jump)), 1000);
    }
    ...
    private final OnSeekBarChangeListener mSeekListener = new OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                ...
                setOverlayProgress();
                mTime.setText(VLCUtil.millisToString(progress));
                showInfo(VLCUtil.millisToString(progress));
            }
    ...
    */
    private final OnPlayerControlListener mPlayerControlListener = new OnPlayerControlListener() {
        ...
        public void onSeekTo(long position) {
            ...
            if(mLibVLC.getLength() <= 0) return;
            mLibVLC.setTime(position);
            mTime.setText(VLCUtil.millisToString(position));
        }
...
//            mOverlayInterface.setVisibility(View.INVISIBLE);
    ...
    private void dimStatusBar(boolean dim) {
        if (!VLCUtil.isHoneycombOrLater() || !VLCUtil.hasNavBar()) {
        	return;
        }
        int layout = 0;
        if (!VLCUtil.hasCombBar() && VLCUtil.isJellyBeanOrLater()) {
        	layout = View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
        }
        mSurface.setSystemUiVisibility((dim ? (VLCUtil.hasCombBar() ? View.SYSTEM_UI_FLAG_LOW_PROFILE : View.SYSTEM_UI_FLAG_HIDE_NAVIGATION) : View.SYSTEM_UI_FLAG_VISIBLE) | layout);
    }
...
//        mControls.setSeekable(length > 0);
        ...
        if (time >= 0) {
        	mTime.setText(VLCUtil.millisToString(time));
        }
        if (length >= 0) {
        	mLength.setText(mDisplayRemainingTime && length > 0 ? "- " + VLCUtil.millisToString(length - time) : VLCUtil.millisToString(length));
        }
        return time;
    }
...
//        mTitle.setText(title);
	...
	public void setSeekable(boolean isSeekable) {
		// TODO Auto-generated method stub
		System.out.println("isseekable = "+isSeekable);
	}
