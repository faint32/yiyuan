...
import com.yiyuan.player.vlc.BitmapCache;
import com.yiyuan.player.vlc.Media;
import com.yiyuan.player.vlc.MurmurHash;
...
public class AudioUtil {
    ...
    public static void setRingtone( Media song, Activity activity){
        File newringtone = VLCUtil.URItoFile(song.getLocation());
    ...
    public static void prepareCacheFolder(Context context) {
        if (VLCUtil.isFroyoOrLater() && VLCUtil.hasExternalStorage() && context.getExternalCacheDir() != null)
    ...
    private static String getCoverFromVlc(Context context, Media media) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        if (artworkURL != null && artworkURL.startsWith("file://")) {
            ...
            if( mArtist.length() == 0 || mAlbum.length() == 0 ||
                mArtist.equals(YiYuanPlayerApplication.getAppContext().getString(R.string.unknown_artist)) ||
                mAlbum.equals(YiYuanPlayerApplication.getAppContext().getString(R.string.unknown_album)) )
            {
    ...
    private static String getCoverFromFolder(Context context, Media media) {
        File f = VLCUtil.URItoFile(media.getLocation());
    ...
    private static Bitmap readCoverBitmap(Context context, String path, int dipWidth) {
        ...
        int width = VLCUtil.convertDpToPx(dipWidth);
